...
from utils.text import DefaultTextProcessing, DictTextProcessing
...
def pad2d_nonzero(x, max_x, max_y):
    ...
    assert max_x >= x.shape[0], f"max_x ({max_x}) < x.shape[0] ({x.shape[0]})"
    assert max_y >= x.shape[-1], f"max_y ({max_y}) < x.shape[-1] ({x.shape[-1]})"
...
def main():
    ...
    parser.add_argument('--symbol_dict', '-d', type=str, default="", help="User defined dictionnary filename")
    ...
    if args.symbol_dict == "":
        text_processing = DefaultTextProcessing(hp.tts_cleaner_names)
    else:
        text_processing = DictTextProcessing(args.symbol_dict)

    # Instantiate Tacotron Model
    print('\nInitialising Tacotron Model...\n')
    model = Tacotron(embed_dims=hp.tts_embed_dims,
                     num_chars=text_processing.get_nb_symbols(),
    ...
    if not force_gta:
        for i, session in enumerate(hp.tts_schedule):
            ...
            train_set, attn_example, max_y, max_x = get_tts_datasets(paths.data, batch_size, r, text_processing)
    ...
    train_set, attn_example,max_y, max_x  = get_tts_datasets(paths.data, 8, model.r, text_processing)
    ...
    for e in range(1, epochs+1):
        ...
        for i, (x, m, ids, _, padded_att_guides) in enumerate(train_set, 1):
            ...
            if attn_example in ids:
                ...
                save_attention(np_now(attention[idx]), paths.tts_attention/f'{step:07d}')
                save_spectrogram(np_now(m2_hat[idx]), paths.tts_mel_plot/f'{step:07d}', 600)

            msg = f'| Epoch: {e}/{epochs} ({i}/{total_iters}) | Loss: {avg_loss:#.4} | m1_loss: {m1_loss:#.4} | m2_loss: {m2_loss:#.4} | Att Loss: {attention_loss:#.4} | {speed:#.2} steps/s | Step: {k}k | '
            stream(msg)
...
def create_gta_features(model: Tacotron, train_set, save_path: Path):
    ...
    for i, (x, mels, ids, mel_lens, _) in enumerate(train_set, 1):
        ...
        with torch.no_grad(): _, gta, _, _ = model(x, mels)
