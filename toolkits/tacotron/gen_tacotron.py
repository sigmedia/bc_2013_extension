...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_file', '-I', type=str, help='[string/path] file containing the utterances to generate!')
    parser.add_argument("--output_dir", "-O", type=str, help="[string//path] output directory path")
    parser.add_argument('--symbol-dict', '-d', type=str, default="", help="[string/path] User defined dictionnary filename")
    ...
    none_parser = subparsers.add_parser('none', aliases=['n'])
    ...
    if input_text:
        inputs = [text_processing.text_to_sequence(input_text.strip())]
        ...
        with open(args.input_file) as f:
            ...
            for i, l in enumerate(f):
                if i == 0:
                    continue
                elts = l.strip().split("\t")
                inputs.append((elts[0], text_processing.text_to_sequence(elts[1])))
    ...
    if args.output_dir:
        ...
        output_dir = paths.tts_output

    for i, cur_utt in enumerate(inputs, 1):

        filename, x = cur_utt
        _, m, attention = tts_model.generate(x)
        m = (m + 4) / 8 # Fix mel spectrogram scaling to be from 0 to 1
        ...
        if input_text:
            ...
            save_mel_path = output_dir/f"{filename}.npy"
        else:
            save_path = output_dir/f"{filename}.wav"
            save_mel_path = output_dir/f"{filename}.npy"
            # save_path = output_dir/f'{i}_{v_type}_{tts_k}k.wav'

        if save_attn: save_attention(attention, save_path)

        # Backup
        np.save(save_mel_path, m)
