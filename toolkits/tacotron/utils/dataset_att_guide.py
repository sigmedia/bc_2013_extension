...
def get_tts_datasets(path: Path, batch_size, r, text_processing):
    ...
    for (item_id, len) in dataset:
        if item_id not in text_dict:
            print(f"Utterance {item_id} not in the dataset")
            continue

        att_path = path/'att_guides'/f'{item_id}.npy'
        if not att_path.exists():
            print(f"Utterance {item_id} does not have any guide")
            continue
    ...
    train_dataset = TTSDataset(path, dataset_ids, text_dict, text_processing)
    ...
    return train_set, attn_example, 400, max(mel_lengths) # FIXME: hardcoded 50


class TTSDataset(Dataset):
    def __init__(self, path: Path, dataset_ids, text_dict, text_processing):
        self.path = path
        self.metadata = dataset_ids
    ...
    def __getitem__(self, index):
        item_id = self.metadata[index]
        x = self.text_processing.text_to_sequence(self.text_dict[item_id])
        mel = np.load(self.path/'mel'/f'{item_id}.npy')
        mel_len = mel.shape[-1]
        att = np.load(self.path/'att_guides'/f'{item_id}.npy')
        # print(f"x.shape = {len(x)}, mel.shape = {mel.shape}, att.shape = {att.shape}")
